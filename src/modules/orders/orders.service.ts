...
export class OrdersService {
  ...
  async getOrdersByTechnId (technId: string): Promise<Order[]> {
    ...
    const orders = await this.ordersRepository.getOrdersByTechnId (technId) || [];

    if (!orders.length) {

      throw new NotFoundException (`No hay órdenes registradas para el técnico con nombre "${technId}"`);
  ...
  async createOrder (createOrderDto: CreateOrderDto): Promise<Order> {
    ...
    const { clientEmail, clientDni, technId, equipmentType, imei, description, status, adminId } = createOrderDto;
      
    const assignedTechnician = await this.usersRepository.findByRole (technId, Role.TECHN);
    ...
    const admin = await this.usersRepository.findByRole (adminId, Role.ADMIN);
